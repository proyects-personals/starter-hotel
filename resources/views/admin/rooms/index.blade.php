        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <h2 class="text-center text-info">Datos de las habitaciones</h2>
    
                    <table class="table-auto w-full text-left bg-white shadow-md rounded">
                        <thead>
                            <tr>
                                <th class="border px-4 py-2">Nombre</th>
                                <th class="border px-4 py-2">Precio por noche</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="border px-4 py-2">{{ $room->hotels->name ?? 'Sin hotel asociado' }}</td>
                                <td class="border px-4 py-2">
                                    @if ($room->image)
                                        <img src="{{ asset('storage/' . $room->image) }}" alt="Imagen de la habitación" width="80">
                                    @else
                                ...
                                <td class="border px-4 py-2">
                                    ...
                                    <form action="{{ route('rooms.destroy', $room->id) }}" method="POST" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn btn-secondary" onclick="return confirm('¿Estás seguro de eliminar esta habitación?')">Eliminar</button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
                                ...
                                <table class="table table-sm table-striped table-hover mb-0">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="text-center text-dark">Precio por noche</th>
                                            <th class="text-center text-dark">Hotel</th>
                                            <th class="text-center text-dark">Imagen</th>
                                    ...
                                    <tbody>
                                        @foreach ($rooms as $room)
                                        <tr class="border-b">
                                            <td class="px-2 py-1 text-center">{{ $room->name }}</td>
                                            <td class="px-2 py-1 text-center">{{ $room->capacity }}</td>
                                            <td class="px-2 py-1 text-center">{{ $room->hotels->name ?? 'Sin hotel asociado' }}</td>
                                            ...
                                            <td class="px-2 py-1 text-center d-flex justify-content-center">
                                                <a href="{{ route('rooms.edit', $room->id) }}" class="btn btn-info btn-sm me-2">Editar</a>
                                                <form action="{{ route('rooms.destroy', $room->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-secondary btn-sm" onclick="return confirm('¿Estás seguro de eliminar esta habitación?')">Eliminar</button>
                                                </form>
                                            </td>
                                        </tr>
                                        @endforeach
