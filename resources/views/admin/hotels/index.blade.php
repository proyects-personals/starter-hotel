            ...
            <tbody>
                @foreach($hotels as $hotel)
                    <tr>
                        ...
                        <td>
                            @if($hotel->image)
                                <img src="{{ asset('storage/' . $hotel->image) }}" alt="Imagen del hotel" width="80">
                            @endif
                        </td>
                        <td>
                            ...
                            <form action="{{ route('hotels.destroy', $hotel->id) }}" method="POST" style="display: inline-block;">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-secundary" onclick="return confirm('¿Estás seguro de eliminar este hotel?')">Eliminar</button>
                            </form>
                        </td>
                    </tr>
                @endforeach
                  ...
                  <div class="card-header pb-0">
                      <div class="row">
                          <div class="d-flex justify-content-center align-items-center">
                              <h2 class="text-center text-info">Lista de Hoteles</h2>
                          </div>
                      </div>
                  </div>
                          ...
                          <table class="table table-sm table-striped table-hover mb-0">
                              <thead>
                                  <tr>
                                      ...
                                      <th class="text-center text-dark">Dirección</th>
                                      <th class="text-center text-dark">Email</th>
                                      <th class="text-center text-dark">Teléfono</th>
                              ...
                              <tbody>
                                  @foreach($hotels as $hotel)
                                  <tr class="border-b">
                                      <td class="px-2 py-1 text-center">{{ $hotel->name }}</td>
                                      <td class="px-2 py-1 text-center">{{ $hotel->address }}</td>
                                      <td class="px-2 py-1 text-center">{{ $hotel->email }}</td>
                                      <td class="px-2 py-1 text-center">{{ $hotel->phone_number }}</td>
                                      <td class="px-2 py-1 text-center">
                                          @if($hotel->image)
                                              <img src="{{ asset('storage/' . $hotel->image) }}" alt="Imagen del hotel" width="60">
                                          @endif
                                      </td>
                                      <td class="px-2 py-1 text-center d-flex justify-content-center">
                                          <a href="{{ route('hotels.edit', $hotel->id) }}" class="btn btn-info btn-sm me-2">Editar</a>
                                          <form action="{{ route('hotels.destroy', $hotel->id) }}" method="POST" class="d-inline">
                                              @csrf
                                              @method('DELETE')
                                              <button type="submit" class="btn btn-secondary btn-sm" onclick="return confirm('¿Estás seguro de eliminar este hotel?')">Eliminar</button>
                                          </form>
                                      </td>
                                  </tr>
                                  @endforeach
