    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900 dark:text-gray-100">
                <h2 class="text-center text-info">Hoteles EKOC</h2>
                    ...
                    <div class="border rounded shadow p-4">
                        <table class="w-full">
                            <tr>
                                <td class="w-1/2">
                                    @if ($hotel->image)
                                        <img class="w-full h-48 object-cover rounded" src="{{ asset('storage/' . $hotel->image) }}" alt="Imagen del hotel">
                                    @else
                                        <img class="w-full h-48 object-cover rounded" src="{{ asset('imagenes/hotel.jpeg') }}"alt="Imagen predeterminada">
                                    @endif
                                </td>
                                <td class="w-1/2 pl-4">
                                    <h2 class="text-lg font-bold">{{ $hotel->name }}</h2>
                                    <p class="text-sm">{{ $hotel->description }}</p>
                                    <p class="text-sm"><strong>Dirección:</strong> {{ $hotel->address }}</p>
                                    <p class="text-sm"><strong>Teléfono:</strong> {{ $hotel->phone_number }}</p>
                                    <p class="text-sm"><strong>Email:</strong> {{ $hotel->email }}</p>
                                    <div class="mt-4">
                                        <a href="{{ route('rooms.show', $hotel->id) }}">
                                            <button class="btn btn-info text-white">Ver Habitaciones Disponibles</button>
                                        </a>
                                    </div>
                                </td>
                            </tr>
          ...
          <div class="col-md-12">
              <div class="card shadow rounded border border-light">
                  <div class="card-header bg-info text-center text-dark rounded-top py-3">
                      <h1 class="mb-1">Hoteles EKOC</h1>
                      <p class="text-white mb-0" style="font-size: 15px;">Explora nuestros hoteles y descubre las mejores opciones para tu estadía</p>
                  </div>
                  <div class="card-body">
                      <div class="row g-4">
                          @foreach ($hotels as $hotel)
                                  ...
                                  <div class="card shadow-sm h-100 rounded border border-muted">
                                      <div class="card-body d-flex flex-column p-3">
                                          <div class="text-center">
                                              @if ($hotel->image)
                                                  <img class="hotel-img rounded" src="{{ asset('storage/' . $hotel->image) }}" alt="Imagen del hotel">
                                              @else
                                                  <img class="hotel-img rounded" src="{{ asset('imagenes/hotel.jpeg') }}" alt="Imagen predeterminada">
                                              @endif
                                          </div>
                                          <h3 class="card-title text-center text-black" style="font-weight: bold; font-size: 20px;">
                                              {{ $hotel->name }}
                                          </h3>
                                          <p class="card-text text-center flex-grow-1">{{ $hotel->description }}</p>
                                          <p class="text-gray-600 text-sm text-center"><strong>Dirección:</strong> {{ $hotel->address }}</p>
                                          <p class="text-gray-600 text-sm text-center"><strong>Teléfono:</strong> {{ $hotel->phone_number }}</p>
                                          <p class="text-gray-600 text-sm text-center"><strong>Email:</strong> {{ $hotel->email }}</p>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        @foreach ($hotels as $hotel)
                            <div class="max-w-sm rounded overflow-hidden shadow-lg bg-white dark:bg-gray-700">
                                <img class="w-full" src="{{ asset('path_to_image.jpg') }}" alt="habitaciones">
                                <div class="px-6 py-4">
                                    <div class="font-bold text-xl mb-2">{{ $hotel->name }}</div>
                                    <p class="text-gray-700 text-base">{{ $hotel->description }}</p>
                                </div>
                                <div class="px-6 pt-4 pb-2">
                                    <!-- Botón para ir a las hanitaciones de las hoteles -->
                                    <a href="{{ route('rooms.show', $hotel->id) }}" class="bg-blue-500 text-white px-4 py-2 rounded">
